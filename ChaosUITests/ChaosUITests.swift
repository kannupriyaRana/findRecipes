...
//  findRecipeUITests
//
//  Created by Administrator on 16/04/22.
//
...
class ChaosUITests: XCTestCase {
    ...
    let duration: Double = 60 * 1 // Execution time limit in seconds
    ...
    func testAddIngredientsScreen() {
        XCUIApplication()/*@START_MENU_TOKEN@*/.staticTexts["search by ingredients"]/*[[".buttons[\"search by ingredients\"].staticTexts[\"search by ingredients\"]",".staticTexts[\"search by ingredients\"]"],[[[-1,1],[-1,0]]],[0]]@END_MENU_TOKEN@*/.tap()
        chaosHandlingForDuration()
        chaosHandlingUntilGestureLimit()
    }
    
    func testLikedRecipesScreen() {
        let app = XCUIApplication()
    ...
    func chaosHandlingUntilGestureLimit() {
